...
通过项目规则或提示入口发现并映射公开的 `skills/experience-distiller/`。它是唯一需要安装或映射的对象；本适配器仅提供接入说明。

不要安装 MCP 或插件，也不要把私有经验库复制到当前公开仓库、Skill 目录或其他事实存储。

## Private repository input

仅在用户明确提供私有经验库根时使用该根。例如，可在提示入口中传入：
...
成功完成会话后，用户从规则或提示入口显式调用 `experience-distiller`，并同时提供私有库根：
